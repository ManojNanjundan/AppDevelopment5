...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    // Retrieve stored email and password from session storage
    const storedEmail = sessionStorage.getItem('email');
    const storedPassword = sessionStorage.getItem('password');
  
    // Validate login based on stored data
    if (emailInput.value === storedEmail && passwordInput.value === storedPassword) {
      // If the entered email and password match the stored data, navigate to the next page
      setNotification("Login successful!");
      clearForm(); // Clear the form on success
      navigate("/user");
    } else {
      // If the entered email and password do not match the stored data, display an error message
